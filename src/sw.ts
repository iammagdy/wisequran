...
// Hashed JS/CSS chunks under /assets/. Precache covers the manifest known
// at build time; this route is a safety net so chunks generated by future
// deploys (or chunks that, for any reason, didn't make it into precache
// — e.g. exceeded the cache budget) still resolve from cache after their
// first visit. CacheFirst is safe because Vite hashes the filename — a
// new chunk version is a different URL, so we never serve stale code.
registerRoute(
  ({ url, request }) =>
    request.destination === "script" || request.destination === "style"
      ...
      : false,
  new CacheFirst({
    cacheName: "wise-app-shell-assets",
    plugins: [
